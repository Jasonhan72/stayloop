...
export async function POST(req: Request) {
  ...
  const { data: lease, error: le } = await sb
    ...
    .select('id, status, terms, tenant_name, tenant_email, unit_label, sign_token, landlord_signature, tenant_signature')
    .eq('id', body.lease_id)
    .maybeSingle()
  ...
  if (!terms?.landlord_legal_name || !terms?.rent?.amount) {
    return NextResponse.json({ error: 'lease terms incomplete — fill the standard form first' }, { status: 422 })
  }
  ...
  const result = await sendEmail({
    ...
    subject: `Your lease for ${unit} is ready to sign — Ontario Standard Lease`,
    text: `Hi ${tenant},

Your landlord has prepared your Residential Tenancy Agreement (Ontario Standard Form of Lease) for ${unit}.
...
你的租约（安省标准租约）已备好，点击上方链接在线查看并签署。签署完成后该链接长期有效，可随时查看和下载备份。

— Stayloop`,
    html: `<p>Hi ${tenant},</p><p>Your landlord has prepared your <b>Residential Tenancy Agreement (Ontario Standard Form of Lease)</b> for <b>${unit}</b>.</p><p><a href="${link}" style="display:inline-block;background:#0f172a;color:#fff;padding:12px 20px;border-radius:8px;text-decoration:none;font-weight:600">Review &amp; sign online →</a></p><p style="color:#64748b;font-size:13px">This link is yours to keep — after signing it remains your permanent access to view and download the agreement anytime.<br/>你的租约已备好，签署完成后该链接长期有效，可随时查看和下载备份。</p><p>— Stayloop</p>`,
  })
  ...
  await admin.from('agent_audit_events').insert({
    ...
    target_id: lease.id,
    metadata: { sent_to: lease.tenant_email, email_id: result.id, form_type: 'ontario_standard' },
  })
