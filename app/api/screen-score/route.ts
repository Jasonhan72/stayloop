...
2. INCOME CORROBORATION — in PERSONAL accounts only, look for RECURRING payroll deposits within ±25% of the claimed monthly income. Emit {claimed_monthly, personal_payroll_seen, observed_pattern, verdict: corroborated|partial|uncorroborated, detail}. If no personal payroll trail exists, verdict=uncorroborated and detail states plainly what WAS observed (e.g. "only a business account receiving ~$18,194/mo from the employer, then transferring $2,000/mo to the applicant"). When verdict=uncorroborated: ability_to_pay MUST NOT exceed 60 and income_stability sub_coverage MUST be "action_pending" (backend enforces both).
   OUTSOURCED PAYROLL: deposits whose payer is a payroll processing provider (OneSource Virtual / OSV for Workday, ADP, Ceridian/Dayforce, Payworks, Wagepoint, Rise, Humi, Nethris, Paychex, Payment Evolution, Deluxe) ARE the employer's payroll — mid/large employers outsource it, and the payer line then carries the processor's name, not the employer's. When the forensics block reports payroll_processor_recognized, a payer ≠ employer name is NOT a mismatch and must not lower verdict, ability_to_pay or verification. An employer name appearing as a counterparty (expense reimbursements) corroborates employment. A single larger payroll deposit that forensics reconciles as bonus_deposit_reconciled is the bonus payout, not a spike.
   NAME ORDER: PR cards, T4 slips, passports and many IDs print SURNAME(S) first ("REGUEIRO RODRIGUEZ CARLOS" = Carlos Regueiro Rodriguez); Hispanic and Portuguese names carry two surnames. Same tokens in a different order are the SAME name — never a discrepancy.
...
- details_en / details_zh: 5 entries each. ≤10 English words per entry. ≤15 Chinese chars per entry. Cite one specific piece of evidence. EXCEPTION — credit_health: when a credit report is present this entry may run to 30 English words / 45 Chinese chars and MUST name the decisive accounts (e.g. 'TD Visa 104% utilised, $5,154 past due; CIBC loan written off').
...
If the uploaded evidence does not support the dimension, score it per the rubric's insufficient-evidence rule and say so in details — do NOT invent a document you were not given.`
    ...
    // Deterministic payroll-pipeline corroboration (lib/forensics/payroll-
    // deposits.ts): the payer is a recognised payroll processor AND the
    // deposits equal the stub net pay to the cent. That is the employer's
    // payroll arriving through an outsourcer — a stronger reading than the
    // model's "payer name differs from employer" (2026-09-11, Acciona / OSV:
    // the model wrote partial + "工资来源不符" on a fully reconciled trail).
    {
      const codes = new Set(forensicsReport.cross_doc_flags.map(f => f.code))
      const ic = crossDocVerification?.income_corroboration
      if (ic && ic.verdict !== 'corroborated' && codes.has('payroll_processor_recognized') && (codes.has('deposits_match_paystub_net') || codes.has('bonus_deposit_reconciled'))) {
