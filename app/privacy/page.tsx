      ...
      <main className="bg-surface">
        <article className="mx-auto max-w-[720px] px-5 py-16 sm:px-7">
          ...
          <Section title={zh ? '4 · 数据存储' : '4 · Data storage'}>
            <p>{zh
              ? '数据库与文件存储在 Supabase 托管的 AWS ca-central-1（加拿大蒙特利尔）区域，边缘计算在 Cloudflare 全球网络。Veriff / Flinks / Equifax（申请人授权时）调用通过加密 TLS 1.3 + token 隔离。'
              : 'The database and file storage run on Supabase in AWS ca-central-1 (Montreal, Canada); edge compute runs on Cloudflare\'s global network. Calls to Veriff / Flinks / Equifax (with the applicant\'s authorisation) run over encrypted TLS 1.3 with token isolation.'}</p>
          </Section>
