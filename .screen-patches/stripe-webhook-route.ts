// -----------------------------------------------------------------------------
// 2026-06-02 — Code review §2 P2 — Add charge.dispute.created and
// charge.refunded handlers; switch to the singleton Stripe client from
// lib/stripe-client. PRESERVES all prior behaviour (idempotency log,
// invoice handlers, subscription handlers, error semantics).
// -----------------------------------------------------------------------------
...
import { getStripe, stripeCryptoProvider } from '@/lib/stripe-client'
 ...
 *           invoice.paid
 *           charge.dispute.created     ← §2 P2 (2026-06-02)
 *           charge.refunded            ← §2 P2 (2026-06-02)
 *
...
export async function POST(req: NextRequest) {
  ...
  try {
    switch (event.type) {
      ...
      // -----------------------------------------------------------------
      // §2 P2 (2026-06-02) — charge.dispute.created
      // -----------------------------------------------------------------
      // A chargeback / payment dispute was opened. We do NOT change `plan`
      // here — we still owe the service for the disputed period and the
      // landlord may win the dispute. We DO flag plan_status='disputed'
      // so the admin dashboard can show a "manual review required" badge
      // and a human can decide whether to revoke access.
      //
      // Sentry warning lets the on-call person triage immediately —
      // disputes are expensive ($15 + fee) and we want to know fast.
      case 'charge.dispute.created': {
        const dispute = event.data.object as Stripe.Dispute
        // Try the inline shape first; if the webhook payload didn't expand
        // `charge`, fall back to a stripe.charges.retrieve() lookup. We
        // never want a dispute to slip through unflagged just because we
        // didn't configure expansion.
        let customerId = extractCustomerFromDispute(dispute)
        if (!customerId && typeof dispute.charge === 'string') {
          try {
            const ch = await getStripe().charges.retrieve(dispute.charge)
            customerId = typeof ch.customer === 'string'
              ? ch.customer
              : ch.customer?.id ?? null
          } catch (lookupErr) {
            captureException(lookupErr, {
              ...
              level: 'warning',
              extra: { event_id: event.id, dispute_id: dispute.id, op: 'charges.retrieve' },
            })
          }
        }
        if (!customerId) {
          captureMessage('charge.dispute.created without customer id', {
            route: 'stripe-webhook',
            level: 'warning',
            extra: { event_id: event.id, dispute_id: dispute.id },
          })
          break
        }
        await admin
          ...
          .eq('stripe_customer_id', customerId)
        captureMessage('Chargeback initiated — manual review needed', {
          route: 'stripe-webhook',
          level: 'warning',
          tags: { event_type: event.type, dispute_reason: dispute.reason || 'unknown' },
          extra: {
            event_id: event.id,
      ...
      // -----------------------------------------------------------------
      // §2 P2 (2026-06-02) — charge.refunded
      // -----------------------------------------------------------------
      // Stripe (or we) refunded a charge. Downgrade plan to 'free' since
      // the customer no longer paid for the period, and mark plan_status
      // 'refunded' so UI / support can distinguish "voluntary cancel"
      // from "money returned". We warn-log so we know it happened — full
      // refunds during a billing period are unusual and worth a glance.
      case 'charge.refunded': {
        ...
        const customerId =
          typeof charge.customer === 'string'
            ...
            : charge.customer?.id
        if (!customerId) {
          captureMessage('charge.refunded without customer id', {
            route: 'stripe-webhook',
            level: 'warning',
            extra: { event_id: event.id, charge_id: charge.id },
          })
          break
        }
        await admin
          .from('landlords')
          .update({ plan: 'free', plan_status: 'refunded' })
          .eq('stripe_customer_id', customerId)
        captureMessage('Stripe charge refunded — plan downgraded', {
          route: 'stripe-webhook',
          level: 'warning',
...
/**
 * Stripe.Dispute carries a `charge` field (id or expanded Charge); the
 * customer id sits on the underlying charge. We accept either shape.
 */
function extractCustomerFromDispute(dispute: Stripe.Dispute): string | null {
  const charge = dispute.charge
  if (!charge) return null
  if (typeof charge === 'string') {
    // Not expanded — the outer handler will fall back to a
    // stripe.charges.retrieve(charge_id) lookup.
    return null
  }
  // charge is the expanded Stripe.Charge here.
  const cust = charge.customer
  if (typeof cust === 'string') return cust
  if (cust && typeof cust === 'object' && 'id' in cust) {
    const id = (cust as { id?: unknown }).id
    return typeof id === 'string' ? id : null
  }
  return null
}
