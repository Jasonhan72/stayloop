// 2026-06-02 — Audit §11 P2 — Replace the CSS background-image hero on
// every listing card with next/image. Stops shipping full-resolution
// realtor.ca / streeteasy / zillowstatic photos straight to the browser;
// next/image now serves an AVIF/WebP variant sized for the card
// (~280-440px wide). Cards stay lazy by default (Image's default loading
// behaviour is "lazy" — explicit `loading="lazy"` would also work but is
// the default), and the `sizes` attribute lets next/image pick a width
// bucket appropriate for the device.
//
// We restructure the photo div: instead of `background: url(...)`, we
// use a relative-positioned container and `<Image fill>` underneath the
// existing overlay chips. The placeholder branch (no heroImage) is
// unchanged — gradient + city emoji.
//
// Companion changes:
//   - next.config.js gains images.remotePatterns for arbitrary HTTPS
//     hostnames (listings come from open-ended third-party CDNs).
//   - app/listings/[slug]/page.tsx swaps its <img> tags for <Image>.
// -----------------------------------------------------------------------------
'use client'
// -----------------------------------------------------------------------------
  ...
  return (
    ...
    >
      {/* Photo area with overlay chips — StreetEasy-style. Hero image when
          ...
          §11 P2 — When a hero image exists we now render it through
          next/image (fill + sizes) instead of as a CSS background. That
          gives us:
            - AVIF/WebP delivery (vs always-PNG from source CDNs)
            - device-bucketed widths (sizes hint below)
            - lazy loading by default (so off-screen cards don't fetch)
          The overlay chips (STAYLOOP badge, save heart) sit on top via
          z-index. Placeholder branch is unchanged. */}
      <div
        style={{
          position: 'relative',
          height: 196,
      ...
      >
        {heroImage ? (
          <Image
            ...
            // unoptimized is the safety net for hosts that don't support
            // the Next image optimizer at runtime (e.g. dev server with
            // an exotic remote). next.config.js's permissive
            // remotePatterns means optimization is normally on.
            // Cards are below the fold — default lazy loading is correct.
          />
        ) : (
          '🏙'
        )}
