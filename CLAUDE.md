...
- **活动面板 = 每段对话一行，不是每条消息一行**（用户同日第二轮：「不是记录每一条消息，是记录每一个对话」，对照 Muse 每行一个 chat +
  ...
  撤销折进对应对话行（计数 + 时间取最晚），没有对话的动作（待办页批准的 cron 卡、工单）各自一行。**行只有一个标题**（第一句用户话，
  单行截断；用户第三轮：「文字太多了，只要做一个标题就可以，点击能到该 chat」）+ 当前对话的「当前」小标；摘要（`agent_threads.summary` =
  助手最后一条回复压成一行 ≤240 字，`threadSummary`）只作桌面端悬停提示，`turn_count` 存着未显示；有执行 / 批准的对话图标 ✓，否则 💬；
  点对话行 = `openThread`。为了能折叠，`thread_id` 现在随卡片走：orchestrator 把它写进
