...
- **Anthropic 100-PDF-page guard** (`lib/anthropic/page-budget.ts`): pre-fetches PDFs, counts pages via pdf-lib, allocates per-kind budget (id=4, pay_stub=4, employment_letter=5, bank_statement=8…), iterates proportional scaling on overage, drops low-priority files entirely if minimums still exceed 95-page safety margin. Truncates long PDFs to base64 (first N-2 + last 2 for >20-page docs to capture summaries). Forensics still runs on full original files; only the Sonnet scoring prompt sees the budgeted version with a `[NOTE]` warning. Fixes "A maximum of 100 PDF pages" error on 20-file uploads.
- **Paystub annual-salary mis-extraction guard**: a legit `$60,000.00/year` salaried pay stub was generating a bogus 207908% period-math error because Haiku put the annual salary into `hourly_rate`. Strengthened EXTRACT_PROMPT with worked examples + added `normalizeExtraction` defensive layer that re-classifies hourly_rate ≥ $10,000 as annual_salary (or drops 200<x<10,000 as noise) before checks run.
- **Ontario Courts Portal surname-position filter**: query "XIONG YI" was matching "ZHENG, YI XIONG" because token-overlap filter didn't enforce surname position. Now parses surname from sortName ("LAST, FIRST") and requires at least one query token to exactly equal it.
- **Portal case-detail link**: hostname `www.courts.ontario.ca` → `courts.ontario.ca` (www form 404s); courtID hardcoded to civil-court UUID instead of reading API's numeric `1`.
- **Rental-history text rewrite when court hits found**: backend enforcement was missing the "0个LTB记录" phrasing. Broadened ZERO_COURT_REGEX_ZH/EN, switched detection from "looks empty" to "explicitly acknowledges hit" — if AI text doesn't mention defendant/被告/case count, always rewrite.
- **Deep-check PRO server gate**: previously client-only gated, anyone with valid session could hit the API. Added `enforceProGate()` that verifies user + queries `landlords.plan` and rejects unless 'pro' or 'enterprise'.
- **Frontend resilience**: AbortController wired into `runAnalysis`, deepcheck stage timers tracked in ref + cleared on unmount. `reset()` now also clears `manualEmployerPrompt`, `manualEmployerName`, `deepCheckStage`, `deepChecking`.
...
- **Image OCR (`lib/forensics/image-ocr.ts`)**: Haiku Vision OCR for image-only PDFs and image files (jpg/png/heic). Triggers when `text_density.is_likely_image_pdf` is true. Output stored on `PerFileForensics.ocr` (text + apparent_doc_type + apparent_name + visible_issuer + has_watermark + visible_dates). Used by id-validation as primary text source for image-only IDs, by cross-doc entity extraction, and by `forensicsToPromptBlock` for Sonnet scoring. (Note: an earlier `pdf-structure.ts` Layer-1 file was claimed shipped but never landed; not currently in use. PDF metadata signals come from `pdf-metadata.ts`.)
