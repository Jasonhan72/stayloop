...
export type AssistantProfile = { name: string | null; avatar: string | null; vibe: string | null }
...
export async function readAssistantProfile(client: SupabaseClient): Promise<AssistantProfile | null> {
  const { data, error } = await client.from('assistant_profiles').select('name, avatar, vibe').maybeSingle()
  if (error) {
    console.warn('[assistant] profile read failed', error.message)
    return null
  }
  if (!data) return null
  const row = data as { name?: string | null; avatar?: string | null; vibe?: string | null }
  return { name: (row.name ?? '').trim() || null, avatar: row.avatar ?? null, vibe: sanitizeVibe(row.vibe) }
}
